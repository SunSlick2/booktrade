...
Function GetTradeDetails(ByVal ws As Worksheet, ByVal dataRow As Long) As Object
    ...
    Dim initialValueDate As String: initialValueDate = tradeDetailsDict("valueDate")
    ...
    parsedValueDate = TryParseDate(tradeDetailsDict("valueDate")) ' Parse once and use result
    ...
    If Len(tradeDetailsDict("tradeType")) = 0 Then
        If Not IsNull(parsedValueDate) Then
            ...
            End If
            tradeDetailsDict("tradeType") = determinedTradeType ' Update the dictionary's tradeType
            tradeDetailsDict("isTradeTypePopulated") = True ' Flag that tradeType was populated
            rawTradeType = LCase(determinedTradeType) ' Update rawTradeType for the Select Case below
        Else
            tradeDetailsDict.Add "Error", "Missing 'tradeType' and unable to infer from 'valueDate'."
            Set GetTradeDetails = tradeDetailsDict
            Exit Function
        End If
    End If
    ...
    Else ' Live Mode
        ...
        If Len(tradeDetailsDict("valueDate")) = 0 Then
            ...
            End Select
        Else ' valueDate is present, verify correctness
            If Not IsNull(parsedValueDate) Then
                Select Case determinedTradeType
                    Case TRADE_TYPE_TOD
                        If parsedValueDate <> todayBiz Then
                ...
                End Select
            Else
                tradeDetailsDict.Add "Error", "Invalid 'valueDate' format for Live trade validation."
                Set GetTradeDetails = tradeDetailsDict
                Exit Function
            End If
        End If
